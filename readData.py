...
def readVar(VAR, path, time_step=1, var2D=False, meta=False, tt=None, xx=None, yy=None, zz=None):
	'''Wrapper for readVariable.
	Checks if saved VAR data has extension .nc or .data.'''

	# If numpy file exists.
	if os.path.isfile(f'{path}{VAR}.npy'):
		return np.load(f'{path}{VAR}.npy')

	# Else, load netcdf or binary data.
...
def readVariable(VAR, path, file_format='nc', time_step=1, var2D=False, meta=False, tt=None, xx=None, yy=None, zz=None):
	...
	if file_format == 'rdmds':
		...
		import numpy as np
		fname = varDict[VAR]['fname']
		return sq(rdmds(path+fname, itrs=np.NaN)[tt[0]:tt[1],zz[0]:zz[1],yy[0]:yy[1],xx[0]:xx[1]])
