...
class Grid:
	...
	def getIceShelf(self, data, iceShelf, areaMean=False):
		...
		if areaMean:
			XX = self.XC; YY = self.YC
			areaIS = self.RAC * (1-self.hFacW[0]) * (1-self.hFacS[0])
			data *= areaIS
			areaIS = np.ma.masked_where(XX<LONS[0], areaIS)
			areaIS = np.ma.masked_where(XX>LONS[1], areaIS)
			areaIS = np.ma.masked_where(YY<LATS[0], areaIS)
			areaIS = np.ma.masked_where(YY>LATS[1], areaIS)
			areaSum = np.ma.sum(areaIS)
		...
		if areaMean:
			return np.ma.sum(data, axis=(-1,-2)) / np.ma.sum(areaSum)
		else:
			return data
	
	#==
	
	def getIceShelfAreaInFront(self, data, iceShelf, areaMean=False, returnArea=False, latsi=None, lonsi=None):

		if iceShelf == 'PIG':
			LATS = [-75., -74.4]; LONS = [254.5, 259]
		elif iceShelf == 'DOT':
			LATS = [-74.2, -73.8]; LONS = [246.2, 248.2]
		else:
			print('grid_PAS.getIceShelfInFront: invalid ice shelf selected')
		...
		YY = self.YC
		areaIS = self.RAC * (1-self.hFacW[0]) * (1-self.hFacS[0])
		if latsi is not None:
			XX = XX[latsi[0]:latsi[1]+1, lonsi[0]:lonsi[1]+1]
			YY = YY[latsi[0]:latsi[1]+1, lonsi[0]:lonsi[1]+1]
			areaIS = areaIS[latsi[0]:latsi[1]+1, lonsi[0]:lonsi[1]+1]
						
		if areaMean or returnArea:
			data *= areaIS
			areaIS = np.ma.masked_where(XX<LONS[0], areaIS)
			areaIS = np.ma.masked_where(XX>LONS[1], areaIS)
			areaIS = np.ma.masked_where(YY<LATS[0], areaIS)
		...
		if len(data.shape) > 2:
			nT, nY, nX = data.shape
			XX = np.tile(XX, (nT,1,1)).reshape((nT,nY,nX))
			YY = np.tile(YY, (nT,1,1)).reshape((nT,nY,nX))
			data = np.ma.masked_where(XX<LONS[0], data)
			data = np.ma.masked_where(XX>LONS[1], data)
			data = np.ma.masked_where(YY<LATS[0], data)
			data = np.ma.masked_where(YY>LATS[1], data)
