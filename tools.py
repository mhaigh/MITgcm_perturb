...
def boundData(data, vmin, vmax, scale=None, d=None):
	...
	if vmin is not None and vmax is not None:
		if scale is not None:
			...
			data = np.ma.where(data>=vmax, vmax-d, data)
			data = np.ma.where(data<=vmin, vmin+d, data)
		else:
...
def ddt(f, dt=1):
	'''A crude time derivative function.
	Assumes t-axis is first.'''
	
	dfdt = np.zeros(f.shape)
	Nt = f.shape[0]

	if isinstance(dt, (np.ndarray, tuple, list)):
		dt_tmp = dt.copy()
	else:
		# Assume dy is scalar.
		dt_tmp = dt * np.ones(f.shape)

	dfdt[1:Nt-1] = (f[2:Nt] - f[0:Nt-2]) / (2 * dt_tmp[1:Nt-1])
	dfdt[0] = (f[1] - f[0]) / dt_tmp[0]
	dfdt[Nt-1] = (f[Nt-1] - f[Nt-2]) / dt_tmp[Nt-1]

	return dfdt
	
#==

def ddx_upw(f, grid, landEqualZero=True):
	'''Zonal derivative for field f on MITgcm c-grid u-points.
	Output is on c-grid C-points.
	If landEqualZero, assume land values are equal to zero (eg for wind curl). NOT IMPLEMENTED YET.'''
	...
	dfdx[...,0:Nx-1] = (f[...,1:Nx] - f[...,0:Nx-1]) / dx[...,0:Nx-1]
	dfdx[...,Nx-1] = dfdx[...,Nx-2]
...
def ddy_upw(f, grid, landEqualZero=True):
	'''Zonal derivative for field f on MITgcm c-grid u-points.
	Output is on c-grid C-points.
	If landEqualZero, assume land values are equal to zero (eg for wind curl). NOT IMPLEMENTED YET.'''
	...
	dfdy[...,0:Ny-1,:] = (f[...,1:Ny,:] - f[...,0:Ny-1,:]) / dy[...,0:Ny-1,:]
	dfdy[...,Ny-1,:] = dfdy[...,Ny-2,:]
...
def ddx(f, dx=1, PERIODIC=False):
	'''A crude zonal derivative function.
	...
	if PERIODIC:
		dfdx[...,0] = (f[...,1] - f[...,Nx-1]) / (2 * dx_tmp[...,0])
		dfdx[...,Nx-1] = (f[...,0] - f[...,Nx-2]) / (2 * dx_tmp[...,Nx-1])
	else:
...
def bottom(data, grid, cellPos, timeDep=True, shiftUp=None, xx=[0,None], yy=[0,None]):
	'''Return value of data at bottom of domain.
	Must specify if at u, v, or h location of grid cells.'''

	if cellPos == 'u':
		hFac = grid.hFacW[:,yy[0]:yy[1], xx[0]:xx[1]]
	elif cellPos == 'v':
		hFac = grid.hFacS[:,yy[0]:yy[1], xx[0]:xx[1]]
	elif cellPos == 'h':
		hFac = grid.hFacC[:,yy[0]:yy[1], xx[0]:xx[1]]
	else:
...
def subtractSurfFlow(data):
	'''Subtract surface flow from data set, assuming surface on axis=1.'''
	
	data_out = data.copy()
	
	for ti in range(data.shape[0]):
		data_out[ti,:] = data_out[ti,:] - data[ti,0]
...
def getIsothermHeight2(temperature_profile, depth_coordinates, target_temperature, kind='linear'):
    """
    Estimates the depth at which the temperature profile equals the target temperature using cubic interpolation.

    Parameters:
        temperature_profile (list): A list of temperatures corresponding to the depth_coordinates.
        depth_coordinates (list): A list of depth coordinates corresponding to the temperature_profile.
        target_temperature (float): The temperature value to find the depth for.

    Returns:
        float: The estimated depth at which the temperature equals the target temperature.
    """
    # Create a cubic interpolation function based on the provided temperature profile and depth coordinates.
    interp_function = interpolate.interp1d(temperature_profile, depth_coordinates, kind=kind)

    # Estimate the depth at which the temperature equals the target temperature.
    estimated_depth = interp_function(target_temperature)

    return estimated_depth
   
#==

def getIsothermDepth3(T, Z, THERM):
	"""
	Finds the shallowest depth at which the vertical temperature profile crosses a reference value.

	Parameters:
		depth_array (numpy.ndarray): 1D array of depth coordinates.
		temperature_field (numpy.ndarray): 3D array (depth, latitude, longitude) containing temperatures.
		reference_temperature (float): The reference temperature value.

	Returns:
		numpy.ndarray: A 2D array (latitude, longitude) containing the shallowest crossing depth for each latitude and longitude.
	"""
    
	ny, nx = T.shape[1], T.shape[2]
	ThermZ = np.zeros((ny, nx), dtype=float)

	for yi in range(ny):
		for xi in range(nx):
		    T_profile = T[:, yi, xi]
		    for zi in range(len(T_profile)-1):
		        if T_profile[zi] <= THERM and T_profile[zi+1] >= THERM and T_profile[zi] < T_profile[zi+1]:
		            ThermZ[yi, xi] = Z[zi+1] + (Z[zi]-Z[zi+1]) * (THERM - T_profile[zi+1]) / (T_profile[zi] - T_profile[zi+1])
		            break
		    else:
		        # If no crossing depth found, set the depth to NaN.
		        ThermZ[yi, xi] = np.nan
...
def getIsohalineDepth3(S, Z, SAL):
	"""
	Finds the shallowest depth at which the vertical salinty profile crosses a reference value.
	"""
    
	ny, nx = S.shape[1], S.shape[2]
	HalZ = np.zeros((ny, nx), dtype=float)

	for yi in range(ny//2):
		for xi in range(nx):
		    S_profile = S[:, yi, xi]
		    for zi in range(len(S_profile)-1):
		        if S_profile[zi] <= SAL and S_profile[zi+1] >= SAL and S_profile[zi] < S_profile[zi+1]:
		            HalZ[yi, xi] = Z[zi+1] + (Z[zi]-Z[zi+1]) * (SAL - S_profile[zi+1]) / (S_profile[zi] - S_profile[zi+1])
		            break
		    else:
		        # If no crossing depth found, set the depth to NaN.
		        HalZ[yi, xi] = np.nan
...
def compute_ocean_pressure(rho, Z, g=9.81):
    '''
    Compute hydrostatic pressure by depth-integrating density.
    
    Parameters:
        rho: 3D array of density (kg/m^3).
        Z: 1D depth array (m).
        g: Gravity (m/s^2), default = 9.81 m/s^2.
        
    Returns:
        P: 3D numpy array of hydrostatic pressure. (Pressure corresponds to bottom of each cell.)
    '''
    
    # Initialize pressure
    P = np.zeros_like(rho)
    
    # Loop over each horizontal position
    for i in range(rho.shape[0]):
        for j in range(rho.shape[1]):
            # Perform numerical integration over depth
            for k in range(1, rho.shape[2]):
                dz = depth[k] - depth[k-1]
                pressure[i, j, k] = pressure[i, j, k-1] + density[i, j, k-1] * g * dz
    
    return pressure

#==

def theta_f(grid, S=None, p=None, rho0=1028.5, g=9.81, nonlinear=True, path=None):
...
def addTimeDim(data, nT):
	'''Add extra dimension of length nT to start of data array.
	Data is copied nT times. Assumes input data has 2 dimensions.'''
	
	nY, nX = data.shape
	
	return np.tile(data, (nT,1,1)).reshape((nT,nY,nX))
	
#==

# spatialFilter2D
def spatialFilter2D(F,fwgp):
